...
if ($stats_link_id > 0) {
    ...
    if ($stats) {
        $tab = 'stats';
        try {
            $q = $pdo->prepare("SELECT * FROM clicks WHERE url_id = ? AND is_bot = 0 ORDER BY id DESC LIMIT 50");
            $q->execute([$stats_link_id]);
            $recent_clicks = $q->fetchAll();
            $q = $pdo->prepare("SELECT device, COUNT(*) as c FROM clicks WHERE url_id = ? AND is_bot = 0 GROUP BY device ORDER BY c DESC");
            $q->execute([$stats_link_id]);
            $by_device = $q->fetchAll();
            $q = $pdo->prepare("SELECT browser, COUNT(*) as c FROM clicks WHERE url_id = ? AND is_bot = 0 GROUP BY browser ORDER BY c DESC");
            $q->execute([$stats_link_id]);
...
.toggle-row .toggle-text{font-size:14px;color:#e2e8f0}
.toggle-row .toggle-sub{font-size:11px;color:#94a3b8;margin-top:2px}
...
<div class="toggle-sub">ON = title + image dikhe · OFF = preview hide</div>
</div>
<label class="switch">
<input type="checkbox" name="preview_enabled" value="1" <?= (!isset($edit_data['preview_enabled']) || (int)($edit_data['preview_enabled'] ?? 1) === 1) ? 'checked' : '' ?>>
...
<a href="?stats=<?= $link['id'] ?>" class="btn-stats">Stats</a>
...
<p style="font-size:12px;color:#94a3b8;margin-bottom:16px;"><?= (int)$stats['clicks'] ?> real clicks</p>
<h2 style="font-size:15px;">By Device</h2>
<?php $maxd = max(array_column($by_device ?: [['c'=>1]], 'c') ?: [1]); foreach ($by_device as $row): $pct = $maxd ? round(($row['c'] / $maxd) * 100) : 0; ?>
<div class="stat-row"><span><?= htmlspecialchars($row['device'] ?: 'Unknown') ?></span><span><?= (int)$row['c'] ?></span></div>
<div class="stat-bar-wrap"><div class="stat-bar" style="width:<?= $pct ?>%"></div></div>
<?php endforeach; ?>
<?php if (empty($by_device)): ?><p class="empty" style="padding:10px 0;">No data yet</p><?php endif; ?>
<h2 style="font-size:15px;margin-top:20px;">By Browser</h2>
<?php foreach ($by_browser as $row): ?>
<div class="stat-row"><span><?= htmlspecialchars($row['browser'] ?: 'Unknown') ?></span><span><?= (int)$row['c'] ?></span></div>
<?php endforeach; ?>
...
<?php endforeach; ?>
<?php if (empty($by_country)): ?><p class="empty" style="padding:10px 0;">No data yet (Cloudflare recommended for country)</p><?php endif; ?>
<h2 style="font-size:15px;margin-top:20px;">Traffic Sources (Referer)</h2>
...
<?php if ($c['country']): ?> · <?= htmlspecialchars($c['country']) ?><?php endif; ?><br>
